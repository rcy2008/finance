...
public class Sender implements MessageListener{
	...
	public void sendMessage(String xml, String signature) throws BankException{
		...
		try {
			...
			msg.setString(XmlConstants.REQ_PARAM_XML, xml);
			msg.setString(XmlConstants.REQ_PARAM_SIGNATURE, signature);
			//设置回复队列
			...
			while (!replyed){
				...
				if(timeOut > XmlConstants.BANK_TRADE_TIMEOUT){
					replyed = true;
					byteData = null;
					throw new BankException("通讯失败");
				}
			}
		} catch (JMSException e) {
			log.error("Sender Create Exception", e);
			throw new BankException("网络异常");
