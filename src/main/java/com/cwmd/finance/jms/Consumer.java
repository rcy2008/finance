...
import com.cwmd.finance.bank.client.NCHttpClient;
...
public class Consumer implements MessageListener, Runnable{
	 ...
	 * @param sendQueue
	 */
	public Consumer(JmsTemplate jmsTemplate, Queue receiveQueue){
		try {
	...
	public void onMessage(Message message) {
		try {
			String xml = ((MapMessage)message).getString(XmlConstants.REQ_PARAM_XML);
			String signature = ((MapMessage)message).getString(XmlConstants.REQ_PARAM_SIGNATURE);
			...
			if (responseQueue != null) {
				...
				byte[] responseByte = NCHttpClient.getHttpResponse(xml, url, signature);
